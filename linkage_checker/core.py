...
def run_linkage_checker_with_selenium(ngr_record, browser_screenshots):
    ...
    linkage_check_results['view_service_linkage'] \
        = query_dom(browser, "thumbs-up", '#resultsOverviewVwAssessment > svg:nth-child(1)')
    # -- Download Service linkage
    linkage_check_results['download_service_linkage'] \
        = query_dom(browser, "thumbs-up", '#resultsOverviewDwAssessment > svg:nth-child(1)')
    ...
    linkage_check_results['data_set_metadata_contains_unique_resource_identifier'] \
        = query_dom(browser, "check-square",
...
def query_dom(browser, search_term, css_selector):
    return search_term in browser.find_element_by_css_selector(css_selector).get_attribute('data-icon')
