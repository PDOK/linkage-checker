...
def get_all_ngr_records(logger, enable_caching):
    ...
    if not os.path.isfile(CACHE_FILENAME) or __cache_is_expired() or not enable_caching:
        ...
        for ngr_record_to_remove in ngr_records_to_remove:
            ngr_records.remove(ngr_record_to_remove)
...
def __get_all_ngr_records(logger):
    ...
    start_position = "1"
    while True:
        records_base_url = NGR_BASE_URL + "/srv/dut/csw-inspire?request=GetRecords&Service=CSW&Version=2.0.2&typeNames" \
                                          "=gmd:MD_Metadata&resultType=results&constraintLanguage=CQL_TEXT" \
                                          "&constraint_language_version=1.1.0&constraint=type='dataset' AND " \
                                          "AnyText+like+'%25" + debug_search + "%25'&startPosition=" + start_position
        ...
        for temp_record in temp_records:
            ...
            temp_record_result = {"uuid_dataset": identifier, "dataset_title": dataset_title}
